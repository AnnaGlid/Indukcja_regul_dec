...
for dataset in ['nursery','crops','lymphography']:
    ...
    def heuristic_v2(all_rules_forest: set, decision: str, alpha: float, rules_per_class: int = 3) -> list:
        assert 0 <= alpha < 1    
        ...
        top_occurences = sorted(desc_occurences.values(), reverse=True)[:rules_per_class]
    ...
    def get_results_for_forest(forest, all_rules_forest, most_common_decision: str, heu: str = 'v1', rules_per_class: int = 3):
        results = {}
            ...
            for decision in class_values:
                if heu == 'v1':
                    ...
                    if get_descriptors(h_rule):
                        rules.append(h_rule)
                if heu == 'v2':
                    h2_rules = heuristic_v2(all_rules_forest, decision, alpha, rules_per_class)                
    ...
    if False:
        #region expreriments: max tree depth v1
        results_depth_rep = []
    ...
    if False:
        #region expreriments: impurity decrease v1
        results_imp_rep = []
        for repeat in range(REPETITION):
                ...
                for imp_decrease in [0, 0.002, 0.004, 0.006, 0.008, 0.01, 0.012, 0.014, 0.016, 0.018, 0.02, 0.022, 0.024, 0.026, 0.028, 0.03]:
                    ...
                    for rules_per_class in [3]:
                        for alpha, forest_results in get_results_for_forest(forest, all_rules_forest, most_common_decision, 'v2', rules_per_class).items():
                            ...
                            results_imp_i['impurity_decrease'].append(imp_decrease)
                            results_imp_i['alpha'].append(alpha)
                            results_imp_i['rules_per_class'].append(rules_per_class)
                            for k, v in forest_results.items():
                                results_imp_i[k].append(v)
            results_imp_rep.append(results_imp_i)
